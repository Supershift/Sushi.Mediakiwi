...
$.widget("ui.multiselect", {
  options: {
	...
	_getOptionNode: function(option) {
		option = $(option);
		var node = $('<li class="ui-state-default ui-element" title="'+option.text()+'"><span class="ui-icon"/>'+option.text()+'<a href="#" class="action"><span/></a></li>').hide();
	...
	_applyItemState: function(item, selected) {
		if (selected) {
			...
			item.find('a.action span').addClass('flaticon solid left-arrow').removeClass('');
			this._registerRemoveEvents(item.find('a.action'));
			
		} else {
			item.children('span').removeClass('ui-icon-arrowthick-2-n-s').addClass('ui-helper-hidden').removeClass('ui-icon');
			item.find('a.action span').addClass('flaticon solid right-arrow').removeClass('');
