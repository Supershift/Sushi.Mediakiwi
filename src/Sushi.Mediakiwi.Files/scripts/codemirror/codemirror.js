//Full screen addon (way below) + Mediakiwi
$(document).ready(function () {
    ...
    $('.SourceCode').each(function (index, element) {
        ...
        function update() {
            editor.setOption("mode", looksLikeX(editor.getValue()) ? "htmlmixed" : "text/x-csharp");
        }
    ...
    function AdjustSize(editor) {
        var fontsize = parseFloat(14);
        var guttersize = 2.071428571 * fontsize + 'px'; 
        var x = parseFloat( (0.0949 * fontsize * fontsize) + (1 * 0.2733 * fontsize) + (1*5.9455) );
        var marginleft = x + 'px';
        var linenumbers = 2 * fontsize + 'px';
        var margintop = '0px';
        // alert('width = ' + guttersize + '\nlinenumbers = ' + linenumbers);
        editor.getWrapperElement().style['font-size'] = fontsize + 'px';
        editor.getGutterElement().style['width'] = guttersize;
        editor.refresh();
        $('.CodeMirror-linenumbers').css('width', linenumbers);
        editor.refresh();
        $('.CodeMirror').css('margin-top', margintop);
        editor.refresh();
        //alert('sizer = ' + $('.CodeMirror-sizer').css('margin-left'));
        $('.CodeMirror-sizer').css('margin-left', marginleft);
        editor.refresh();
    };
});
