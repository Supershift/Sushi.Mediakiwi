...
namespace Sushi.Mediakiwi.Logic
{
    public class OAuth2Logic
    {
        ...
        public OAuth2Logic(ConfigurationManager<OpenIdConnectConfiguration> openIdConfigurationManager)
        {
        ...
        public async Task<string> ExtractUpnAsync(AuthenticationConfiguration authenticationConfiguration, string idtoken)
        {
            try
            {
                ...
                // get config, like signing keys
                var openIdConnectConfigData = await _openIdConfigurationManager.GetConfigurationAsync();
