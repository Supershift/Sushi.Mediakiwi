    ...
    public class DateTimeAttribute : ContentSharedAttribute, IContentInfo
    {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(Console.Form(this.ID)))
                {
                    try
                    {
                        ...
                        {
                            if (!string.IsNullOrEmpty(Console.Form(this.ID + "T")))
                            {
                                string[] timearr = Console.Form(this.ID + "T").ToString().Split(':');

                                m_Candidate = tmp.AddHours(Data.Utility.ConvertToInt(timearr[0])).AddMinutes(Data.Utility.ConvertToInt(timearr[1]));
                            }
                            else
                                m_Candidate = tmp;
                        }
        ...
        public Field WriteCandidate(WimControlBuilder build, bool isEditMode, bool isRequired, bool isCloaked)
        {
            ...
            if (isEditMode)
            {
                ...
                element.AppendFormat("\n\t\t\t\t\t\t\t\t\t<input class=\"date datepicker{3}\" name=\"{0}\" type=\"text\" id=\"{0}\" maxlength=\"10\" value=\"{1}\" placeholder=\"{2}\"/>"
                    , this.ID
                ...
                element.AppendFormat("\n\t\t\t\t\t\t\t\t\t<input class=\"time timepicker{3}{4}{5}\" type=\"text\" name=\"{0}T\" maxlength=\"5\" placeholder=\"00:00\" value=\"{1}\" />{2}"
                   , this.ID
